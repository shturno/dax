...
describe('FeedbackPage CRUD', () => {
  it('should add a new feedback', async () => {
    ...
    await waitFor(() => {
      ...
      const found = Array.from(cards).some(card => 
        card.textContent && card.textContent.includes('Feedback Teste')
      );
  ...
  it('should edit a feedback', async () => {
    ...
    await waitFor(() => expect(screen.queryAllByRole('menuitem', { hidden: true }).length).toBeGreaterThan(0));
    ...
    await waitFor(() => {
      ...
      const found = Array.from(updatedCards).some(card => 
        card.textContent && card.textContent.includes('Feedback Editado')
      );
      expect(found).toBe(true);
    }, { timeout: 5000 });
  }, 15000);
  ...
  it('should filter feedbacks', async () => {
    ...
    // Filter by 'Melhorar'
    fireEvent.change(screen.getByPlaceholderText(/Buscar feedback/i), { target: { value: 'Melhorar' } });
